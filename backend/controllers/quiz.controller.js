...
export const generateQuiz = async (req, res) => {
  try {
    ...
    if (existingQuiz && !req.query.regenerate) {
      console.log("Found existing quiz:", existingQuiz._id);
    ...
    try {
      ...
      console.log("Generating quiz using OpenAI API...");
      const prompt = `Generate a multiple-choice quiz about ${moduleTitle}, specifically focused on the lesson "${lessonTitle}". 
      Create 5 questions with 4 options each and make sure these questions are advance level. Return the response in the following JSON format:
      {
        "questions": [
          {
            "id": "q1",
    ...
    console.log("Sending sanitized quiz to client");
...
export const submitQuizResponses = async (req, res) => {
  try {
    ...
    const userId = req.user._id;
    console.log("User ID:", userId, "Quiz ID:", quizId);

    // Find the quiz
    const quiz = await Quiz.findOne({ _id: quizId, userId });
    
    if (!quiz) {
      console.log("Quiz not found for user", userId, "and quizId", quizId);
    ...
    console.log("Saving quiz responses with score:", score);
    await quiz.save();
    
    // Calculate percentage
    const percentage = Math.round((score / quiz.totalQuestions) * 100);

    console.log("Quiz completed. Final score:", score, "/", quiz.totalQuestions);
    res.status(200).json({
      _id: quiz._id,
      score,
      totalQuestions: quiz.totalQuestions,
...
export const getQuizResults = async (req, res) => {
  try {
    ...
    console.log("Getting quiz results for quiz:", quizId, "user:", userId);
    const quiz = await Quiz.findOne({ _id: quizId, userId });
